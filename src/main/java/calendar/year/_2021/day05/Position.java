...
public record Position(int x, int y, Part part) {

    public List<Position> computeIntermediatePositions(Position position) {
        ...
        if (this.x == position.x) {
            ...
            intermediatePositions.addAll(IntStream.range(start, end)
                    .mapToObj(yPos -> new Position(this.x, yPos, part))
                    .toList());
            ...
            intermediatePositions.addAll(IntStream.range(start, end)
                    .mapToObj(xPos -> new Position(xPos, this.y, part))
                    .toList());
        ...
        if (!intermediatePositions.isEmpty()) {
            intermediatePositions.add(new Position(this.x, this.y, part));
            intermediatePositions.add(new Position(position.x(), position.y(), part));
        }
    ...
    private Set<Position> computeDiagonals(Position position) {
        ...
        while (currentX != position.x()) {
            diagonals.add(new Position(currentX, currentY, part));
            currentX += xIncrement;
            currentY += yIncrement;
        }
